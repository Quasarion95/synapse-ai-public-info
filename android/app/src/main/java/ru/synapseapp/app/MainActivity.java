...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /* Пересобранной копии ассистент не достаётся.

           Подписать чужим ключом пересборку невозможно — ключ только у нас, —
           поэтому чужая подпись означает перезалитую сборку. Ругаться и падать
           смысла нет: кто пересобирал, тот и проверку вырежет. Просто не
           говорим странице, что она внутри настоящего приложения: без этого
           признака она не попросит у сервера сессию Syn, а ответ на запрос с
           чужого источника бэкенд и так не даст. */
        if (Подлинность.своя(this)) {
            web.addJavascriptInterface(new Object() {
                @android.webkit.JavascriptInterface
                public boolean genuine() { return true; }
            }, "AndroidGenuine");
        }
