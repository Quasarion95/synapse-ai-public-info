...
var PALETTES = [
  ...
  /* Тёмная половина «Бумаги» — цвета тёмной темы сайта (#1D2420 и её текст):
     бурый тёмный фон владелец назвал тем, чем он был. Светлая половина не
     тронута. */
  { id: 'paper', title: 'Бумага', lift: 4.5, v: [247,245,239,29,36,32,47,39,20,245,241,232,121,102,57,207,209,198,216,205,177,58,68,61,93,78,46,132,162,140,138,110,53,198,164,105,47,90,114,130,176,201,63,107,82,127,191,155,132,97,31,211,168,92] },
...
/* Разбор строки: «кофе 350», «350 кофе», «зарплата 90 000», «такси 1.5к».
   ...
   заполняет три поля. */
/* Валюта и повисший предлог — не часть названия траты.

   «кафе 500р» оставляло после вырезанной суммы «кафе р», «кафе 500 рублей» —
   «кафе рублей», а «кафе за 500» — «кафе за». Человек диктует так, как
   говорит, и рубли произносит вслух.

   Это перенос stripMoneyWords из приложения: там баг починили в августе, а в
   вебке и апк он дожил до сих пор — просто потому, что разбор строки написан
   дважды и правку внесли в одну половину. Нашли сценарной проверкой. */
function stripMoneyWords(title){
  var text = String(title || '')
    .replace(/(?:^|(?<![а-яёa-z0-9]))(?:рубл[а-яё]*|руб\.?|р\.?|₽|rub|rur)(?![а-яёa-z])/gi, ' ')
    // «за», «на», «около» держались за сумму, которой больше нет.
    .replace(/(?:^|(?<![а-яё]))(?:за|на|по|около|примерно|где-?то)\s*$/i, '')
    .replace(/\s+/g, ' ');
...
/// «Переносилась 4 раза», а не «4×»: значок понимал только автор.
/// Русское согласование: 2–4 — «раза», дальше — «раз» (с поправкой на 12–14).
function ruTimes(count){
  var tail = count % 10, hundred = count % 100;
  var word = (tail >= 2 && tail <= 4 && (hundred < 12 || hundred > 14)) ? 'раза' : 'раз';
  return 'Переносилась ' + count + ' ' + word;
}
...
function itemRow(t, внутриЦели){
  ...
  if ((t.carried || 0) >= 2) meta.push('<span class="chip carried">' +
    ruTimes(t.carried) + '</span>');
  ...
  /* draggable не ставим нигде.

     Раньше на компьютере карточки таскали нативным HTML5-перетаскиванием, и
     атрибут был нужен ему. Теперь перенос везде ведёт единый указательный
     движок (pointerdown/move/up ниже) — одинаково для мыши, пальца и
     приложения. А draggable="true" ему МЕШАЕТ: браузер на нажатии с движением
     запускает свой системный drag, рисует полупрозрачную копию карточки и
     забирает себе поток указательных событий — обработчик перестаёт получать
     pointermove, и наш перенос замирает на месте. Отсюда «тень-клон ходит, а
     карточка стоит». На сенсоре его давно убрали по этой причине; на
     компьютере он оставался по недосмотру и ровно так же ломал перенос мышью. */
...
function vProfile(){
  ...
  /* Блока «Подписка» здесь больше нет — владелец попросил убрать дубль:
     в меню есть отдельный раздел «Моя подписка», и одна и та же информация
     в двух местах читалась как две разные. */
  return html;
}
...
function synLogHTML(){
  ...
  return S.synChat.map(function(item, index){
    ...
    /* Плашки «ничего не изменилось» здесь больше нет.

       Она задумывалась как честность: Syn ответил словами, ничего не сделал —
       и интерфейс об этом говорил, предлагая повторить или разобрать фразу
       самому. На деле она всплывала и там, где человек просто СПРОСИЛ, а не
       просил что-то изменить: на вопрос «что у меня на сегодня» действий и не
       должно быть, а плашка всё равно кричала о поломке и лезла под руку.
       Ложная тревога надоедает быстрее, чем помогает редкий настоящий случай.

       Обе кнопки остались живы (data-act syn-local и syn-again) — если плашку
       когда-нибудь вернут, показывать её нужно только когда человек ПРОСИЛ
       действие, а не задавал вопрос. */
...
var ACTS = {
  ...
  'fin-opening-save': function(){
    ...
    /* Ноль — законный остаток: человек начинает с нуля, и это не то же самое,
       что «не вписал». finParse нули отбрасывает — для трат это правильно,
       для остатка нет, поэтому ноль ловим до него. Подсказку «сначала — сколько
       у вас есть» при этом гасим тем же флагом, что и её крестик: иначе она
       вернулась бы, потому что ноль для неё неотличим от пустоты. */
    if (/^\s*[-−]?\s*0+(?:[.,]0{1,2})?\s*(?:₽|р\.?|руб[а-яё.]*)?\s*$/i.test(raw)){
      S.finance.opening = 0;
      S.finance.openingHidden = true;
      closeModal();
      commit('Записано');
      return;
    }
  ...
  expand: function(d){
    ...
    /* Раскрываем или сворачиваем — сбрасываем свайп.

       Свайп у раскрытой карточки теперь работает, но держать его открытым во
       время перестройки высоты незачем: панель меряется по верхней строке в
       момент касания, а при сворачивании строка та же, только карточка вокруг
       другая. Проще начать жест заново, чем следить за согласованностью. */
...
document.addEventListener('pointerdown', function(event){
  // Единый путь для мыши, пальца и приложения: браузерного перетаскивания
  // больше нет, всё ведёт этот движок.
...
document.addEventListener('pointerdown', function(event){
  // Цель события не всегда элемент: у document и текстовых узлов closest нет,
  // и без этой проверки один посторонний pointerdown ронял весь свайп.
  var цель = event.target;
  if (!цель || typeof цель.closest !== 'function') return;
  // Нажали по самим кнопкам под карточкой — это не свайп, а выбор действия.
  if (цель.closest('.side')) return;
  var карточка = цель.closest('.swipe');
  if (!карточка) { swipeCloseAll(null); return; }
  /* Раскрытую карточку тоже свайпаем.

     Панель кнопок растянута на всю высоту карточки, поэтому у раскрытой её
     раньше просто прятали — и удалить задачу свайпом было нельзя, пока не
     свернёшь. Теперь панель у раскрытой встаёт по верхней строке (правило
     .item.swipe.open > .side), а высоту этой строки меряем здесь: в CSS её
     не вычислить, название бывает и в две строки. */
  var лицоКарточки = карточка.querySelector('.swipe-face');
  if (карточка.classList.contains('open') && лицоКарточки){
    карточка.style.setProperty('--swipe-side-h', лицоКарточки.offsetHeight + 'px');
  }
