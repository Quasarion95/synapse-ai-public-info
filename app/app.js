...
function местоВставки(zone, clientY, ход){
  ...
  if (ход.пик === undefined) ход.пик = clientY;
  if (ход.низ === true)  ход.пик = Math.max(ход.пик, clientY);
  if (ход.низ === false) ход.пик = Math.min(ход.пик, clientY);

  function можно(внизСейчас){
    if (ход.низ === undefined) return true;
    if (ход.низ === внизСейчас) return true;
    return Math.abs(ход.пик - clientY) >= ОТКАТ_ПЕРЕСТАНОВКИ;
  }

  // Вниз: порог — ВЕРХНЯЯ грань соседа снизу.
  while (idx < cards.length &&
         clientY >= baseTop(cards[idx]) + КРАЙ_ПЕРЕСТАНОВКИ && можно(true)){
    idx++; ход.низ = true; ход.пик = clientY;
  }

  // Вверх: порог — НИЖНЯЯ грань соседа сверху.
  while (idx > 0 &&
         clientY <= baseTop(cards[idx - 1]) + cards[idx - 1].offsetHeight - КРАЙ_ПЕРЕСТАНОВКИ &&
         можно(false)){
    idx--; ход.низ = false; ход.пик = clientY;
  }
